...
Prism.languages.python={comment:{pattern:/#.*/,greedy:!0},string:{pattern:/(?:[rb]|rb|br)?(?:"{3}(?:[\s\S]*?)"{3}|'{3}(?:[\s\S]*?)'{3}|(?:"(?:"(?!"")|[^"\\]|\\.)*"|'(?:'(?!'')|[^'\\]|\\.)*'))/i,greedy:!0},"function-definition":{pattern:/((?:\b(?:async|def)\b\s+)|(?:\bdef\b\s+))(?!\d)\w+(?=\s*\()/,lookbehind:!0,alias:"function"},"class-name":{pattern:/(\bclass\s+)\w+/,lookbehind:!0},keyword:/\b(?:and|as|assert|async|await|break|class|continue|def|del|elif|else|except|exec|finally|for|from|global|if|import|in|is|lambda|nonlocal|not|or|pass|print|raise|return|try|while|with|yield)\b/,function:/\b(?!\d)\w+(?=\s*\()/,boolean:/\b(?:False|None|True)\b/,number:/\b(?:\d+(?:\.\d+)?(?:[eE][+-]?\d+)?|0x[\da-fA-F]+|0b[01]+|0o[0-7]+)\b/,operator:/[-+*/%=<>!=&|^~]|==|\*\*|\/\/|<<|>>/,punctuation:/[(){}\[\];:,.]/};