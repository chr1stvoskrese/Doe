...
Ты Senior Backend Developer с 20-ти летним стажем
Ты Senior UI/UX Designer с 20-ти летним стажем
Абсолютный эксперт в этих трех областях

Мой проект называется Doe
Doe — локальное десктопное канбан-приложение для macOS с хранением данных в выбранной пользователем папке (как Obsidian). Бэкенд на FastAPI + SQLAlchemy + SQLite, фронтенд на чистом HTML/CSS/JavaScript, упаковка в .app через pywebview.
...
Примечание о pywebview
На данный момент обёртка pywebview не добавлена в проект, так как мы находимся на этапе отладки бэкенда и фронтенда. После завершения основной функциональности будет написан отдельный скрипт (wrapper.py), который:
Запустит FastAPI в фоновом потоке.
Создаст окно pywebview с URL http://127.0.0.1:порт/app.
Реализует лаунчер выбора хранилища (отдельное окно с выбором/созданием папки).
Сейчас приложение запускается как обычный веб-сервер и доступно по адресу http://127.0.0.1:8000/app.
"""
...
def get_prompt():
    ...
    print("\n" + "=" * 50)
    print("Что нужно сделать нейросети? (ЗАДАЧА)")
