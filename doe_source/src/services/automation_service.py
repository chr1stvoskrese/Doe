...
async def _execute_sort_column(db: AsyncSession, auto: AutomationModel) -> Optional[int]:
    ...
    # Получаем все задачи в колонке, отсортированные по целевому полю
    result = await db.execute(
        select(TaskModel)
    ...
    for i, task in enumerate(tasks):
        expected_pos = float(i + 1)
        if abs(task.position - expected_pos) > 0.001:
            needs_sort = True
            break
    ...
    # Переназначаем позиции: 1.0, 2.0, 3.0 ...
    for i, task in enumerate(tasks):
        task.position = float(i + 1)
