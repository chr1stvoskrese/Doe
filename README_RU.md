...
**🧠 Локальный AI** — Встроенный AI-ассистент на базе **Gemma 4** — работает полностью оффлайн, с ускорением на Apple Silicon. Общается о задачах, ищет по доске, создаёт карточки, помнит факты между сессиями.
...
main.py              # FastAPI-приложение (in-process ASGI)
